 ...
 */
class TunerResults(val size: Int, val sampleRate: Int) {
    fun set(workingData: WorkingData){
        require(workingData.size == size)
        require(workingData.sampleRate == sampleRate)

        workingData.correlation.copyInto(correlation)
        workingData.ampSqrSpec.copyInto(ampSqrSpec)
        pitchFrequency = workingData.pitchFrequency
        noise = workingData.noise
    }
