...
fun Modifier.dragZoom(
    state: Plot2State, transformation: () -> Transformation,
...
//            val decay = exponentialDecay<Rect>(1f)
            detectPanZoomFlingGesture(
                ...
                onGesture = { centroid, pan, zoom ->
                    ...
                    state.setViewPort(
                        newViewPortRaw,
                        Plot2State.TargetTransitionType.Snap,
                        Plot2State.BoundsMode.Gesture
                    )
                },
                onFling = { velocity ->
                    val velocityRaw = transformation().toRaw(velocity)
                    state.flingViewPort(velocityRaw.copy(x = 0f), Plot2State.BoundsMode.Gesture)
//                    val t = transformation()
...
//            val decay = exponentialDecay<Rect>(1f)
            detectPanZoomFlingGesture(
                ...
                onFling = { velocity ->
                    ...
                    val velocityRaw = transformation().toRaw(velocity)
                    state.flingViewPort(velocityRaw.copy(y = 0f), Plot2State.BoundsMode.Gesture)
//                    state.viewPortRawAnimation.animateDecay(
...
//            val decay = exponentialDecay<Rect>(1f)
            detectPanZoomFlingGesture(
                ...
                onFling = { velocity ->
                    val velocityRaw = transformation().toRaw(velocity)
                    state.flingViewPort(velocityRaw, Plot2State.BoundsMode.Gesture)
//                    val t = transformation()
