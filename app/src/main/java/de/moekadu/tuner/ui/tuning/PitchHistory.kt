...
class PitchHistoryState(
    ...
    private var count = FloatArray(initialCapacity) { it.toFloat() }
        private set
    private var _values = ResizeableArray(initialCapacity)
        private set
    ...
    val plotState = PlotState(
        Rect(0f, 500f, capacity * 1.05f, 400f),
        Rect(0f, 20000f, capacity  * 1.05f, 10f)
    )
        ...
        plotState.setYTicks(
            ...
            maxLabelHeight = {
                computeMaxNoteSize(
                    ...
                    fontSize = 12.sp,
                    fontWeight = null,
                    octaveRange = musicalScale.getNote(musicalScale.noteIndexBegin).octave .. musicalScale.getNote(musicalScale.noteIndexEnd).octave,
    ...
    fun addFrequency(value: Float) {
        _values.add(value)
        plotState.setLine(KEY_LINE, count, values, 0, size)
        plotState.setPoint(
            KEY_CURRENT_POINT,
            Offset((size-1).toFloat(), value),
            Point.drawCircle(8.dp, {MaterialTheme.colorScheme.primary}) // TODO: define other color, point size must come from somewhere else
        )
    }
...
//            private set
    ...
    companion object {
        ...
        const val KEY_LOWER_TOLERANCE = 5
        const val KEY_UPPER_TOLERANCE = 6
    }
}
